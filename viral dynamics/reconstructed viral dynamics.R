...
##################################################################
qcolors <- c("black","#00a0e9","#f65294","#009944")
ObserveData <- fread("monolix_estimated_parameter/3VOC//viralload_observations.txt")
IndiPars <- fread("monolix_estimated_parameter/3VOC/estimatedIndividualParameters.txt")

data_list <- vector(mode="list",length=length(IndiPars$id))
for (i in 1:length(data_list)){
  target_id <- IndiPars$id[i]
  target_data <- ObserveData[ObserveData$ID == target_id, ]
  target_data <- target_data[target_data$viralload > 0, ]
  data_list[[i]] = data.frame(time = target_data$time, value = target_data$viralload)
}
...
for(i in 1:length(IndiPars$id)){
  ...
  plt <- ggplot() +
    geom_point(data = data_list[[i]], aes(x=time, y=value), color=curvecolor, size=0.5, shape=16, stroke = 2.5) +
...
ObserveData <- fread("monolix_estimated_parameter/del_omi//viralload_observations.txt")
